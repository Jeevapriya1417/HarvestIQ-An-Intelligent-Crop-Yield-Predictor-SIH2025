...
import { TrendingUp, CheckCircle, AlertCircle, Leaf, Beaker, Cloud, ArrowRight, ArrowLeft, Sparkles, Wind, Droplets, Thermometer } from 'lucide-react';
import { useApp } from '../context/AppContext';
import Navbar from './Navbar';
import { Button, Input, Select, Card, ProgressBar, LoadingSpinner, Alert } from './ui';
import predictionEngine from '../services/predictionEngine';
import { predictionAPI } from '../services/api';
import { getAgriculturalWeatherData } from '../services/weatherService'; // Added weather service import
...
const PredictionForm = () => {
  ...
  const [isWeatherLoading, setIsWeatherLoading] = useState(false); // Added weather loading state
  ...
  const [weatherData, setWeatherData] = useState(null); // Added weather data state
  const [weatherError, setWeatherError] = useState(null); // Added weather error state
  ...
  // Python ML model only supports these 6 crops
  // Other crops are auto-mapped to these by backend
  const SUPPORTED_CROPS = ['Wheat', 'Rice', 'Sugarcane', 'Cotton(lint)', 'Maize', 'Barley'];
  ...
  // Function to fetch weather data based on region
  const fetchWeatherData = async (region) => {
    ...
    try {
      // Map region to a more specific location for better weather data
      const locationMap = {
      ...
      const location = locationMap[region] || region;
      const result = await getAgriculturalWeatherData(location);
      
      if (result.success) {
        setWeatherData(result);
        // Auto-fill weather fields if they are empty
        setFormData(prev => ({
          ...prev,
          rainfall: prev.rainfall || result.data?.precipitation || '',
          temperature: prev.temperature || result.data?.temperature || '',
          humidity: prev.humidity || result.data?.humidity || ''
        }));
      } else {
  ...
  // Effect to fetch weather when region changes
  useEffect(() => {
    if (formData.region && currentStep === 3) {
      fetchWeatherData(formData.region);
    }
  }, [formData.region, currentStep]);
  ...
  const generatePrediction = async () => {
    ...
    try {
      ...
      // Try backend API first (Python ML model)
      try {
        console.log('\n🔄 [FRONTEND] Calling backend API for prediction...');
        const response = await predictionAPI.createPrediction({
          inputData: {
            cropType: inputData.cropType,
            farmArea: parseFloat(inputData.farmArea),
            region: inputData.region,
            phLevel: inputData.phLevel ? parseFloat(inputData.phLevel) : null,
            organicContent: inputData.organicContent ? parseFloat(inputData.organicContent) : null,
            nitrogen: inputData.nitrogen ? parseFloat(inputData.nitrogen) : null,
            phosphorus: inputData.phosphorus ? parseFloat(inputData.phosphorus) : null,
            potassium: inputData.potassium ? parseFloat(inputData.potassium) : null,
            rainfall: parseFloat(inputData.rainfall),
            temperature: parseFloat(inputData.temperature),
            humidity: inputData.humidity ? parseFloat(inputData.humidity) : null
          },
          aiModel: {},
          field: null
        });
        ...
        if (!response.success) {
          throw new Error(response.message || 'Backend prediction failed');
        }
        
        // Backend returns: { success, data: { prediction: {...} } }
        result = response.data.prediction || response.data;
        console.log('✅ [FRONTEND] Backend prediction successful');
        console.log('   - Model Type:', result.aiModel?.modelType || 'unknown');
        console.log('   - Model Name:', result.aiModel?.modelName || 'unknown');
        console.log('   - Processing Time:', result.processing?.processingTime || 'N/A', 'ms');
        
      } catch (backendError) {
        console.warn('⚠️  [FRONTEND] Backend API failed:', backendError.message);
        console.log('🔄 [FRONTEND] Falling back to local prediction engine...');
        
        // Fallback to local JavaScript engine
        result = await predictionEngine.generatePrediction(inputData);
        console.log('✅ [FRONTEND] Local fallback prediction successful');
      }
      
      // Debug logging
      console.log('\n📊 [FRONTEND] Final Prediction Result:', result);
      ...
      const predictionRecord = {
        ...
        results: {
          expectedYield: result?.results?.expectedYield || result?.expectedYield || 'N/A',
          yieldPerHectare: result?.results?.yieldPerHectare || result?.results?.expectedYield || result?.expectedYield || 'N/A',
          totalYield: result?.results?.totalYield || (parseFloat(result?.results?.expectedYield || result?.expectedYield || 0) * parseFloat(inputData.farmArea || 1)).toFixed(2),
          confidence: result?.results?.confidence || result?.confidence || 95,
          recommendations: result?.recommendations || [],
          modelUsed: result?.aiModel?.modelName || 'JavaScript Generic Model'
        },
        processing: {
          status: 'completed',
              ...
              <div className="space-y-8">
                  ...
                  <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <Select
                        ...
                        onBlur={handleBlur}
                        options={cropOptions.filter(crop => SUPPORTED_CROPS.includes(crop.value))}
                  ...
                  <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <Input
                        ...
                        onBlur={handleBlur}
                        placeholder="e.g., 1.5"
                        hint="Typical range: 0.5% - 2.5% (soil organic matter)"
                ...
                {currentStep === 3 && (
                  <div className="animate-fade-in">
                    ...
                    {isWeatherLoading && (
                      <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-center space-x-3">
                        ...
                        <span className="text-blue-700 dark:text-blue-300">
                          Fetching current weather data for {formData.region}...
                        </span>
                      </div>
                    )}
                    ...
                    {weatherError && (
                      <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg flex items-center space-x-3">
                    ...
                    {weatherData && !isWeatherLoading && (
                      <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <h3 className="font-semibold text-green-800 dark:text-green-200 mb-2 flex items-center">
                          ...
                          Current Weather in {weatherData.data?.location?.name || formData.region}
                        </h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="font-semibold">{weatherData.data?.temperature}°C</p>
                            </div>
                          </div>
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="font-semibold">{weatherData.data?.humidity}%</p>
                            </div>
                          </div>
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="font-semibold">{weatherData.data?.precipitation || '0'} mm</p>
                            </div>
                          </div>
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="font-semibold">{weatherData.data?.condition || 'N/A'}</p>
                            </div>
                          </div>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                          * Weather data has been automatically filled. You can modify these values if needed.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="mt-4 flex justify-end">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => fetchWeatherData(formData.region)}
              ...
              <div className="text-center space-y-6">
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl p-6 max-w-md mx-auto">
                  ...
                  <p className="text-3xl font-bold text-green-600 dark:text-green-400">
                    {prediction?.results?.expectedYield || prediction?.expectedYield || 'N/A'} tons/ha
                  </p>
                  <p className="text-sm text-green-700 dark:text-green-300 mt-2">
                    Confidence: {prediction?.results?.confidence || prediction?.confidence || 95}%
                  </p>
                  {(prediction?.results?.totalYield || prediction?.totalYield) && (
                    <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                      Total: {prediction?.results?.totalYield || prediction?.totalYield} tons
                    </p>
                  )}
                </div>
