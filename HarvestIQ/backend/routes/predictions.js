...
const validatePredictionInput = [
  body('inputData.cropType')
    .isIn(['Wheat', 'Rice', 'Sugarcane', 'Cotton', 'Maize', 'Barley', 'Mustard', 'Potato', 'Onion', 'Tomato'])
  ...
  body('inputData.soilData.phLevel')
    .optional()
    .isFloat({ min: 0, max: 14 })
    .withMessage('pH level must be between 0 and 14'),
  
  body('inputData.weatherData.temperature')
    .optional()
    .isFloat()
    .withMessage('Temperature must be a valid number'),
  
  body('inputData.weatherData.rainfall')
    .optional()
    .isFloat({ min: 0 })
...
export const createPrediction = async (req, res) => {
  try {
    ...
    if (aiModel?.modelId) {
      ...
      if (!selectedModel || !selectedModel.status.isActive) {
        return res.status(400).json({
          success: false,
      ...
      // Use default model for the crop type
      selectedModel = await AiModel.findDefault() || await AiModel.findOne({ 
        type: 'javascript',
